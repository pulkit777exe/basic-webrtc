    ...
    >
      <div className="flex items-center justify-between p-4 sm:p-5">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-[var(--room-text)]">Participants</h3>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-4 sm:p-5">
        ...
        {participants.map((p) => (
          <div
            key={p.userId}
            className="flex items-center gap-3 rounded-xl bg-[var(--room-elevated)] px-3 py-2"
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--room-border)] text-xs font-semibold text-[var(--room-text)]">
              {p.user.name.slice(0, 2).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-[var(--room-text)]">
                {p.user.name}
                {p.userId === user?.id && ' (You)'}
              </p>
              <p className="text-xs capitalize text-[var(--room-muted)]">{p.role}</p>
            </div>
            {p.role === 'host' && (
