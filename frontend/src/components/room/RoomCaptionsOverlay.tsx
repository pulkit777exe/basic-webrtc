...
export function RoomCaptionsOverlay() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-24 left-1/2 z-30 w-[min(720px,calc(100vw-2rem))] -translate-x-1/2 sm:bottom-28">
      <div className="pointer-events-auto max-h-40 overflow-y-auto rounded-xl border border-[var(--room-border)] bg-[var(--room-header)]/95 px-3 py-2 backdrop-blur-md">
        {captions.length === 0 ? (
          <p className="text-center text-xs text-[var(--room-muted)]">Listening for speech…</p>
        ) : (
          <div className="space-y-1.5">
            {captions.map((caption) => (
              <p key={caption.id} className="text-sm text-[var(--room-text)]">
