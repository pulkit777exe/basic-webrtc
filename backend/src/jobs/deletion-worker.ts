...
export async function runDeletionJob(userId: string, deletionRequestId: string): Promise<void> {
...
export function startDeletionWorker() {
  ...
  const worker = new Worker<DeletionJobData>(
    'account-deletion',
    async (job) => {
      await runDeletionJob(job.data.userId, job.data.deletionRequestId);
    },
    {
      connection: conn,
      concurrency: 1,
